...
void CMenu::drawSignUpMenu(string& username, string& password) {
	//initialize
	if (username.empty()) {
		SetConsoleCursorPosition(window, { 0,0 });
		drawBorder(80, 8);
...
void CMenu::drawSignNotification(int type) {
	if (type == 1) {
...
string CMenu::getPassword(string username, string password) {
	char c = 0;
...
string CMenu::getUsername(string username, string password) {
	char c = 0;
	do {			// 13 in ASCII is Enter
		c = _getch();
		if (c == 8) {			// 8 in ASCII is Backspace
