...
BEGIN_MESSAGE_MAP(CMFCMainDlg, CDialogEx)
    ...
    ON_BN_CLICKED(IDC_REFRESH_BUTTON, &CMFCMainDlg::OnBnClickedRefreshButton)
END_MESSAGE_MAP()
...
void CMFCMainDlg::OnBnClickedDownloadButton()
{
    ...
    if (client.GetFileFromServer(filename_str, folder_str)) {
        MessageBox(_T("Download Success!!"));
    }
    else {
        string Error = client.GetLastError();
        CString error_cstr(Error.c_str());
        MessageBox(error_cstr);
    }
}
...
{
    // TODO: Add your control notification handler code here
    if (!client.isUploadAllowed()) {
        MessageBox(
        (LPCWSTR)L"Another user is uploading!! Please wait!!",
        (LPCWSTR)L"Error",
        MB_ICONWARNING);
        return;
    }
...
void CMFCMainDlg::OnBnClickedRefreshButton()
{
    listFileCtrl.ResetContent();
    string fileDB;
    client.GetDB(fileDB);
    ifstream db(fileDB);
    string temp;
